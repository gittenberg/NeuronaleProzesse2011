...
openingPoints = np.where(np.diff(shtCmdVoltage) > 4)[0]
openingPoints *= tmpShtRes
...
for group in EPSCs:
    ...
    for x in range(0, len(group[0])):
        averagePSC.append(np.average(np.array(group)[:,x]))
